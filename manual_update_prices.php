...
// Get all ProBookingCenter tours with periods but no price
$tours = DB::table('tb_tour as t')
    ->join('tb_tour_period as p', 't.id', '=', 'p.tour_id')
    ->where('t.wholesale_id', 39)
    ->whereNull('t.deleted_at')
    ->whereNull('p.deleted_at')
    ->where('t.price', 0)
    ->where('p.price1', '>', 0)
    ->select('t.id', 't.code1', 't.price', 'p.price1', 'p.special_price1')
    ->distinct()
    ->get();

echo "Found " . $tours->count() . " ProBookingCenter tours with periods but no tour price\n\n";
...
// Check Tour ID 125035
$tour125035 = DB::table('tb_tour')->where('id', 125035)->first(['id', 'code1', 'price', 'special_price', 'price_group']);
echo "\n=== Tour ID 125035 ===\n";
echo "  Code1: {$tour125035->code1}\n";
echo "  Price: {$tour125035->price}\n";
echo "  Special Price: {$tour125035->special_price}\n";
echo "  Price Group: {$tour125035->price_group}\n";
